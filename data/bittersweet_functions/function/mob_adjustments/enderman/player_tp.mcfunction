...
execute as @e[type=minecraft:enderman,sort=nearest,limit=1] at @s run tp @e[type=minecraft:player,tag=PullTarget,sort=nearest,limit=1] ^ ^ ^
particle minecraft:portal ~ ~1 ~ 1 1 1 0.5 30
particle minecraft:end_rod ~ ~2 ~ 1 1 1 0.5 10

execute as @e[type=minecraft:enderman,sort=nearest,limit=1] as @s run scoreboard players set @s PullTimer 0
execute as @e[type=minecraft:enderman,sort=nearest,limit=1] as @s run effect clear @s slowness
